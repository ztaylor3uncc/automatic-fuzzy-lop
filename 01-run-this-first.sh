...
if [ -d /usr/share/focs/firmware-library/*$file.extracted ]; then
	echo -e "${ERROR}This binary already exists in the library.${NC}"
	echo -e "${ERROR}Run the binary without the target to fuzzz.${NC}"
	echo -e "${ERROR}Exiting...${NC}"
	exit 1;
fi
...
sudo mv $file /usr/share/focs/ || { echo -e "${ERROR}Couldn't move to the /usr/share/focs directory... check that the directory exists.${NC}" && exit 1; };
